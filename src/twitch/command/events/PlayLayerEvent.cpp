...
void PlayLayerEvent::setPlayerScale(int playerIdx, float scale, float time) {
    Loader::get()->queueInMainThread([playerIdx, scale, time] {
        log::debug("[PlayLayerEvent] setPlayerScale called with playerIdx={}, scale={}, time={}", playerIdx, scale, time);
        auto playLayer = PlayLayer::get();
        if (!playLayer) {
            log::error("[PlayLayerEvent] setPlayerScale: PlayLayer not found");
            return;
        }
        auto player = (playerIdx == 2) ? playLayer->m_player2 : playLayer->m_player1;
        log::debug("[PlayLayerEvent] setPlayerScale: playLayer={}, m_player1={}, m_player2={}, selectedPlayer={}",
            (void*)playLayer, (void*)playLayer->m_player1, (void*)playLayer->m_player2, (void*)player);
        if (!player) {
            log::error("[PlayLayerEvent] setPlayerScale: Player {} not found (nullptr)", playerIdx);
            return;
        }
        float startScale = player->getScale();
        log::debug("[PlayLayerEvent] setPlayerScale: startScale={}, targetScale={}, duration={}", startScale, scale, time);
        if (time > 0.0f) {
            ...
            public:
                ...
                void update(float dt) override {
                    ...
                    if (target) {
                        ...
                        log::debug("[PlayerScaleAnimScheduler] setScale: {} (elapsed: {}, t: {})", newScale, elapsed, t);
                    }
                    if (t >= 1.f) {
                        if (target) {
                            target->setScale(toScale);
                            log::debug("[PlayerScaleAnimScheduler] Final setScale: {}", toScale);
                        }
            ...
            if (scene) {
                ...
                log::debug("[PlayLayerEvent] PlayerScaleAnimScheduler started for player {}", playerIdx);
            } else {
                log::error("[PlayLayerEvent] Could not get running scene for animation");
                player->setScale(scale);
                log::debug("[PlayLayerEvent] Fallback: player->setScale({}) called immediately", scale);
            }
        } else {
            player->setScale(scale);
            log::debug("[PlayLayerEvent] setPlayerScale: player->setScale({}) called immediately", scale);
        }
